...
/* JONGQ header file declartion start */
#include <sys/types.h>
#include <string.h>
#include <fcntl.h>
#include <sys/stat.h>
...
#include <string.h>
#include <sys/mman.h>
/* JONGQ header file declartion finish */

bool NVDIMM_FLAG=true; /* JONGQ SWITCH OFF => false */
bool LOG_ALLOCATE_FLAG=false;
bool NVDIMM_HALF_FLAG=false;
void* nvdimm_save = NULL;
int nvdimm_fd = 0;
...
# endif
	...
	/* JONGQ Mapping NVDIMM to Redo LogBuffer */
	if(NVDIMM_FLAG){
		nvdimm_fd = open("/dev/pmem0", O_RDWR|O_DIRECT);
		if(nvdimm_fd < 0){
			fprintf(stderr,"[ERROR] NVDIMM Mapping Fail\n");
			return(srv_init_abort(err));
		}
	
		nvdimm_save = (void *) mmap(NULL, 13*1024*1024*1024, PROT_READ | PROT_WRITE, MAP_SHARED, nvdimm_fd, 0x40008000);
		if (nvdimm_save == MAP_FAILED){
		...
		fprintf(stderr,"[JONGQ] NVDIMM Setting Success!\n");
	}
