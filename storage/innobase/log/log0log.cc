...
bool checkBufHalf(void){
	if(log_sys->buf_free > log_sys->buf_size/2){
		...
		fprintf(stderr,"[JONGQ] FALSE buf_free: %lu \t buf_size: %lu \n",log_sys->buf_free,log_sys->buf_size);
	return false;
}
...
/*==========*/
{
	...
	LOG_ALLOCATE_FLAG=true; /* JONGQ SWITCH OFF => false */
...
{
	// JONGQ
	//fprintf(stderr,"[JONGQ] log_group_close()\n");
...
{
	// JONGQ
	// fprintf(stderr,"[JONGQ] log_group_close_all()\n");
	log_group_t*	group;
...
{
	// JONGQ
	// fprintf(stderr,"[JONGQ] log_shutdonw call!\n");
...
{
	// JONGQ
	// fprintf(stderr,"[JONGQ] log system data free\n");
	if (log_sys != NULL) {
		recv_sys_mem_free();
		ut_free(log_sys);
